...
async function getDetails(placeId) {
  const fields = 'place_id,name,formatted_address,formatted_phone_number,website,geometry,rating,opening_hours,photos,types,address_components';
...
async function run() {
  const searches = [
    { slug: 'wolf-bay-restaurant', q: 'Wolf Bay Restaurant Foley Alabama' },
    { slug: 'ginny-lane-bar-grill', q: 'Ginny Lane Bar Grill Gulf Shores Alabama' },
  ];

  for (const { slug, q } of searches) {
    console.log('\nSearching:', q);
    const candidates = await searchPlace(q);
    if (!candidates.length) { console.log('  NO RESULTS'); continue; }
    ...
    for (const tag of tags) {
      await db.from('entity_tags').insert(tag);
    }
    ...
    if (det.opening_hours && det.opening_hours.periods) {
      ...
      for (const p of det.opening_hours.periods) {
        ...
        if (day) {
          await db.from('entity_hours').upsert(
            { entity_slug: slug, day_of_week: day, open_time, close_time },
