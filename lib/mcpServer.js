 ...
 * @param authenticate (req) => caller, or { reason } to refuse with 401. Omit for
 *                     a server that needs no identity at all; return a plain
 *                     object without `reason` to accept one optionally.
 * @param authNote     what /info should say about credentials, when
 *                     "there is an authenticate function" is the wrong summary
 *                     — an optional identity is not a requirement.
 */
function createMcpRouter({ serverInfo, instructions, tools, runTool, authenticate, authNote }) {
        ...
        switch (method) {
            case 'initialize': {
                ...
                return ok(id, {
                    ...
                    serverInfo,
                    // await, because the slug-attached server names the business
                    // and the public one folds in what it remembers about the
                    // caller — both are reads, not constants.
                    instructions: typeof instructions === 'function' ? await instructions(caller) : instructions,
                });
            }
    ...
    router.get('/info', (_req, res) => {
        res.json({
            ...
            authentication: authNote || (authenticate ? 'Authorization: Bearer <token>' : 'none — public'),
