...
const CONCIERGE_TOOLS = [
    ...
    },
    {
        ...
        description:
            'Which businesses have capacity left on a date, across the whole coast — "who has availability for a dolphin cruise today", "any fishing charters open Saturday", "who can take four people this afternoon". Returns the businesses with open slots, how many are left, and at what time. Only businesses that publish their availability appear; say who does rather than implying the rest are full.',
        inputSchema: {
            type: 'object',
            properties: {
                what: {
    ...
    if (name === 'find_available') {
        ...
        const [capacity, slots, blocks] = await Promise.all([
            db.from('business_availability')
                ...
                .eq('availability_date', date).neq('status', 'full').limit(500),
            db.from('availability')
                .select('entity_slug, date, start_time, end_time, status, spots_total, spots_remaining')
                .eq('date', date).gt('spots_remaining', 0).limit(500),
            db.from('booking_calendar')
                .select('entity_slug, date, end_date')
                .eq('kind', 'block').is('offering_id', null).neq('status', 'cancelled')
                .lte('date', date).limit(500),
        ]);
        if (capacity.error) return { error: capacity.error.message };

        const blocked = new Set(
            (blocks.data || [])
                .filter((b) => b.date <= date && (!b.end_date || b.end_date >= date))
                .map((b) => b.entity_slug),
        );

        const open = new Map();
        const add = (slug, slot) => {
            if (blocked.has(slug)) return;
            if (party && slot.spots != null && slot.spots < party) return;
            if (!open.has(slug)) open.set(slug, []);
            open.get(slug).push(slot);
        };
        for (const r of (capacity.data || [])) {
            add(r.entity_slug, { time: r.time_slot || null, until: r.end_time || null, spots: r.remaining_spots ?? null, of: r.total_capacity ?? null });
        }
        for (const r of (slots.data || [])) {
            add(r.entity_slug, { time: r.start_time || null, until: r.end_time || null, spots: r.spots_remaining ?? null, of: r.spots_total ?? null });
        }
        ...
        let q = db.from('entity')
            .select('slug, name, entity_type, entity_subtype, city, phone, rating, price_from, price_unit, description')
            .eq('is_active', true)
            .in('slug', [...open.keys()]);
        ...
        return {
            ...
            note: available.length
                ...
                : `Businesses have capacity on ${date}, but none matching "${what}". Widen the search rather than saying nobody is open.`,
        };
    }
