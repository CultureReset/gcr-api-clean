...
/**
 * A bounded slice of the catalogue, so one HTTP request can finish.
 *
 * listToolkits() above walks every page before it returns anything — for the
 * thousand-odd toolkits Composio carries that is a dozen sequential round
 * trips, and a serverless function is killed long before the last one lands.
 * The caller never sees an error, just a dropped connection.
 *
 * This fetches at most `pages` pages from `cursor` and hands back where it got
 * to, so the work can be spread across as many requests as it takes. Pass the
 * returned cursor to continue; a null cursor means the catalogue is exhausted.
 */
async function listToolkitsPage({ cursor, pages = 3, query } = {}) {
    const out = [];
    let next = cursor || undefined;

    for (let page = 0; page < Math.max(1, pages); page += 1) {
        const payload = await composio('/toolkits', { query: { ...query, limit: 100, cursor: next } });
        const items = payload?.items || payload?.data || (Array.isArray(payload) ? payload : []);
        out.push(...items);
        next = payload?.next_cursor || payload?.nextCursor || null;
        if (!next) break;
    }

    return { toolkits: out.map(normaliseToolkit), nextCursor: next || null };
}
