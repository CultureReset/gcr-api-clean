...
function usedCapacity(claims, options) {
    ...
    for (const claim of claims || []) {
        ...
        const claimEnd = toDate(claim.end_date) || claimStart;
        if (date < claimStart || date > claimEnd) continue;
...
function quote(input) {
    ...
    const days = product.schedule_mode === 'date_range' ? datesBetween(date, endDate, 370).length || 1 : 1;
    const nights = product.schedule_mode === 'date_range' ? nightsBetween(date, endDate) : 1;
    ...
    for (const item of requested) {
        ...
        const multiplier = rateMultiplier(rate, { durationMinutes: product.duration_minutes, days: days, nights: nights });
        const unitCents = toCents(rate.amount);
...
module.exports = {
    ...
    availabilityForDate, availabilityForRange,
    // pricing
