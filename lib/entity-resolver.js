...
async function resolveEntity(req) {
  try {
    ...
    // 1b. Platform/BookPro accounts store the businesses.id (siteId) in
    // entity_owners.user_id (see routes/platform.js ownedSlug) — try that too
    if (req.siteId) {
      const { data: owner } = await db
        .from('entity_owners')
        .select('entity_id, entity_slug')
        .eq('user_id', req.siteId)
        .maybeSingle();
      if (owner && owner.entity_id) {
        return { id: owner.entity_id, slug: owner.entity_slug };
      }
