...
async function resolveEntity(req) {
  try {
    // 0. Admin view-as token (middleware/auth.js sets this from a JWT minted
    //    by POST /api/admin/view-as) — scopes the whole dashboard to that
    //    entity so an admin can open any business's live dashboard.
    if (req.viewAsEntitySlug) {
      const { data: entity } = await db
        .from('entity')
        .select('id, slug')
        .eq('slug', req.viewAsEntitySlug)
        .maybeSingle();
